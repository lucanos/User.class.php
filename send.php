...
<body>
	<div id="container">
		...
		<?php if($user->isLoggedIn()) { if(!isset($_POST['to'])) {?>
		<?php if(isset($_GET['id'])) { $to = $user->userInfoId($_GET['id']);}?>
		...
		<?php } else { 
			...
			if($user->exists(mysql_real_escape_string($_POST['to'])) && $user->sendMessage($to['id'], $from['id'], mysql_real_escape_string($_POST['subject']), $_POST['message'])) {
				echo '<div class="Sendtext">Sent! Click <b><a href="messages.php">here</a></b> to return</div>';
			} else {
				echo '<div class="Sendtext">Error. User doesnt exist.</div>';
			}
		}}?>
	</div>
</body>
